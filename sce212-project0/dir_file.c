...
void make_dir_and_file(directory_t* root_dir, char** token_list, int num_tokens) {
    ...
    directory_t *child_dir = NULL; 
    file_t *child_file = NULL;
    ...
    child_file = find_create_file(current_dir,token_list[num_tokens-1],&is_create);
    current_dir->file_list[current_dir->num_files] = child_file;
